...
//		}
		...
		public function fetchDeferredUri(onFetched:Function):void
		{
			if (_initialized) {
				...
				_context.call("fetchDeferredUri", getNewCallbackName(onFetched, "fetchDeferredUri"));
			} else {
		...
		private function getNewCallbackName(callback:Function, name:String = ""):String
		{
			// Generate callback name based on current time
			var date:Date = new Date();
			var callbackName:String = date.time.toString() + name;
...
//        log("onInvoke reason: " + event.reason + " params: " + debugStr);
		...
		private function onStatus(event:StatusEvent):void
		{
			...
			else if (event.code.indexOf("DEFERRED") != -1) {

				log("Got deferred from app store");
